...
def create_mapping_examples() -> List[Tuple[str, CubeToSimplexMapping]]:
    ...
    mappings.append(("Symmetric", CubeToSimplexMapping(
    ...
    mappings.append(("Asymmetric", CubeToSimplexMapping(
    ...
    mappings.append(("Sharp", CubeToSimplexMapping(
    ...
    mappings.append(("Smooth", CubeToSimplexMapping(
...
def figure_1_lattice_mappings():
    ...
    for col, (name, mapping) in enumerate(mappings):
        ...
        scatter = ax_simplex.scatter(p1, p2, c=p3, cmap='plasma', alpha=0.7, s=15,
                                   vmin=0, vmax=1)

        # Draw simplex boundary
        triangle = patches.Polygon([(0,0), (1,0), (0,1)], fill=False,
                                 edgecolor='black', linewidth=2)
        ax_simplex.add_patch(triangle)

        # Shade simplex interior
        triangle_fill = patches.Polygon([(0,0), (1,0), (0,1)], alpha=0.1,
                                      facecolor='gray')
...
def figure_2_3d_visualization():
    ...
    scatter1 = ax1.scatter(X.ravel(), Y.ravel(), np.zeros_like(X.ravel()),
                          c=range(len(X.ravel())), cmap='viridis', alpha=0.7, s=25)
    ax1.set_xlabel('$x_1$')
    ax1.set_ylabel('$x_2$')
    ax1.set_zlabel('$z = 0$')
    ...
    ax3.scatter(X.ravel(), Y.ravel(), np.zeros_like(X.ravel()),
               c='blue', alpha=0.5, s=20, label='Cube')
    ...
    for i in indices:
        ...
        ax3.plot3D([x_start, x_end], [y_start, y_end], [0, z_end + 1],
    ...
    plt.tight_layout()
    fig.suptitle('Figure 2: Three-Dimensional Visualization of Cube-to-Simplex Mapping',
                fontsize=16, y=1.02)
...
def figure_3_quality_analysis():
    ...
    for i in range(len(names)):
        for j in range(len(param_names)):
            ax3.text(j, i, f'{param_matrix[i, j]:.2f}',
                    ha='center', va='center', color='white' if abs(param_matrix[i, j]) > 1 else 'black')
    ...
    im4 = ax4.imshow(jacobian_dets, extent=[margin, 1-margin, margin, 1-margin],
    ...
    hexplot = ax5.hexbin(simplex_samples[:, 0], simplex_samples[:, 1],
                        C=simplex_samples[:, 2], gridsize=20, cmap='viridis',
                        extent=[0, 1, 0, 1])
    ...
    for alpha in alpha_values:
        ...
        metrics = comprehensive_quality_assessment(
            test_mapping,
            symmetry_samples=1500, volume_samples=100, smoothness_samples=100,
            coverage_samples=1000, invertibility_samples=15, random_seed=42
        )
...
def figure_4_optimization_results():
    ...
    for alg, result in results.items():
        ...
        ax1.plot(evaluations, running_max, color=colors[alg],
                linewidth=2, label=f'{alg.title()} Search')
    ...
    bars = ax2.bar([alg.title() for alg in alg_names], final_scores,
    ...
    scatter = ax3.scatter(simplex_points[:, 0], simplex_points[:, 1],
                         c=simplex_points[:, 2], cmap='plasma', alpha=0.7, s=20)
    ...
    bars = ax4.bar(quality_names, quality_values,
...
def main():
    ...
    fig1.savefig(f"{output_dir}/figure_1_lattice_mappings.png",
    ...
    fig2.savefig(f"{output_dir}/figure_2_3d_visualization.png",
                bbox_inches='tight', facecolor='white')
    ...
    fig3.savefig(f"{output_dir}/figure_3_quality_analysis.png",
                bbox_inches='tight', facecolor='white')
    ...
    fig4.savefig(f"{output_dir}/figure_4_optimization_results.png",
                bbox_inches='tight', facecolor='white')

...
from thurstone.adaptive_special_horse import (
    AdaptiveSpecialHorse, SpecialHorseConfig, DistributionType
)
...
class SpecialHorseStudyManager:
    ...
    def __init__(self, output_dir: str = "special_horse_results"):
        ...
        self.quality_weightings = {
            ...
            'symmetry_coverage': {'symmetry': 2.0, 'volume_preservation': 1.0, 'smoothness': 1.0,
                                 'coverage': 2.0, 'invertibility': 1.0},
            'coverage_first': {'symmetry': 1.0, 'volume_preservation': 1.0, 'smoothness': 1.0,
...
def run_special_horse_study(k: int = 2):
    ...
    total_combinations = (len(distributions) * len(base_abilities) *
                ...
                for scale in scales:
                    for adaptive_mode in adaptive_modes:
                        ...
                        if eval_count % 10 == 0:
                            ...
                            remaining = (total_combinations - eval_count) / rate / 60 if rate > 0 else 0
                            print(f"   Progress: {eval_count}/{total_combinations} ({100*eval_count/total_combinations:.1f}%) "
    ...
    for weighting_name in study.quality_weightings.keys():
        ...
        for i, result in enumerate(top_10, 1):
            ...
            print(f"   {i:2d}. {config['distribution']:<10} {config['adaptive_mode']:<15} "

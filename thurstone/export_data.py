...
def export_mapping_data(mapping,
                       resolution: int = 20,
                       include_quality: bool = True,
                       quality_samples: Optional[Dict[str, int]] = None) -> Dict[str, Any]:
    """
    ...
    if hasattr(mapping, 'sigmoid_params'):
        ...
        if hasattr(mapping, 'special_horse_ability'):
            ...
            export_data['configuration']['special_horse'] = {
                'distribution': config.distribution.value if hasattr(config.distribution, 'value') else str(config.distribution),
...
def export_mapping_to_file(mapping,
                          filename: str,
                          resolution: int = 20,
...
def export_parameter_study_data(parameter_configs: List[Dict[str, Any]],
                               resolution: int = 10,
                               output_file: str = 'parameter_study.json') -> str:
    """
    ...
    for i, config in enumerate(parameter_configs):
        ...
        if ENHANCED_AVAILABLE and 'special_horse' in config:
            # Enhanced mapping
            from .adaptive_special_horse import SpecialHorseConfig, AdaptiveSpecialHorse, DistributionType

            special_config_dict = config['special_horse'].copy()
            if 'distribution' in special_config_dict and isinstance(special_config_dict['distribution'], str):
                # Convert string to enum
                special_config_dict['distribution'] = DistributionType(special_config_dict['distribution'])
            ...
            mapping = CubeToSimplexMapping(sigmoid_params=sigmoid_params, special_horse_ability=special_ability)

        # Export this configuration
        config_data = export_mapping_data(mapping, resolution, include_quality=True,
                                        quality_samples={'symmetry_samples': 50, 'volume_samples': 10,
                                                       'smoothness_samples': 10, 'coverage_samples': 50,
...
def create_example_exports():
    ...
    symmetric_mapping = CubeToSimplexMapping(sigmoid_params=symmetric_params, special_horse_ability=0.0)
    ...
    asymmetric_mapping = CubeToSimplexMapping(sigmoid_params=asymmetric_params, special_horse_ability=0.4)
    ...
    if ENHANCED_AVAILABLE:
        ...
        enhanced_mapping = EnhancedCubeToSimplexMapping(sigmoid_params=symmetric_params, special_horse=enhanced_horse)

...
def plot_cube_lattice_2d(mapping: CubeToSimplexMapping,
                        resolution: int = 20,
...
def plot_cube_to_simplex_3d(mapping: CubeToSimplexMapping,
                           resolution: int = 15,
    ...
    ax1.scatter(X.ravel(), Y.ravel(), np.zeros_like(X.ravel()),
    ...
    ax3.scatter(X.ravel(), Y.ravel(), np.zeros_like(X.ravel()),
               c='blue', alpha=0.4, s=15, label='Cube points')
    ...
    for i in indices:
        ...
        ax3.plot3D([x_cube, x_simp], [y_cube, y_simp], [0, z_simp + 1],
...
def plot_jacobian_heatmap(mapping: CubeToSimplexMapping,
                         resolution: int = 30,
                         epsilon: float = 1e-4,
    ...
    im1 = ax1.imshow(jacobian_dets, extent=[margin, 1-margin, margin, 1-margin],
    ...
    im2 = ax2.imshow(log_dets, extent=[margin, 1-margin, margin, 1-margin],
                     origin='lower', cmap='plasma', aspect='equal')
    ax2.set_xlabel('x₁')
    ax2.set_ylabel('x₂')
...
def plot_quality_summary(mapping: CubeToSimplexMapping,
    ...
    ax2.axhline(y=expected_prob, color='red', linestyle='--',
    ...
    if 'jacobian_determinants' in volume_details:
        ...
        ax3.axvline(x=np.mean(jacobian_dets), color='red', linestyle='--',
    ...
    if 'gradient_norms' in smoothness_details:
        ...
        ax4.axvline(x=np.mean(gradient_norms), color='red', linestyle='--',
    ...
    if mapping.k == 2:
        ...
        ax5.scatter(simplex_samples[:, 0], simplex_samples[:, 1],
...
def visualize_mapping_comprehensive(mapping: CubeToSimplexMapping,
                                  resolution: int = 20,
